...
| ---- | ---- |
| ![CI Checks](https://github.com/byu-oit/<%= repoName %>/workflows/CI%20Checks/badge.svg?branch=master) | ![CI Checks](https://github.com/byu-oit/<%= repoName %>/workflows/CI%20Checks/badge.svg?branch=dev) |
| ![CD Pipeline](https://github.com/byu-oit/<%= repoName %>/workflows/CD%20Pipeline/badge.svg?branch=master) | ![CD Pipeline](https://github.com/byu-oit/<%= repoName %>/workflows/CD%20Pipeline/badge.svg?branch=dev) |
...
4) AWS Setup *(this will create Route53 hosted zones in your aws accounts and upload SSM params)*
    ...
    3) Run terraform to setup the DEV environment
        ...
        3) Use [this order form](https://it.byu.edu/it/?id=sc_cat_item&sys_id=2f7a54251d635d005c130b6c83f2390a) to request having your dev subdomain added to BYU's DNS servers.
           Add yourself as the technical contact, select 'NS Records', and list the NS records found in Route 53 (from above step) in the "Aliases" field. They also should have been output at the end of the `terraform apply` step.
...
7) Push your changes to your GitHub repo master branch (and dev branch)
    * this should start the pipeline workflows (dev and prd), which will each take 15-30 minutes to spin up the CloudFront distributions
...
With Tyk you can specify multiple callback urls for a given client. You'll probably want to add `https://localhost:3000` as one of yours to the dev client so that testing locally works.
